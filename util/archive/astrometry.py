...
from astropy.coordinates import (BaseCoordinateFrame, frame_transform_graph,
                                 CartesianRepresentation, CartesianDifferential, ICRS, Attribute, Galactocentric)
    ...
    def __init__(self, obj, tracer=None):
        ...
        if obj == 'LMC':
            if tracer == 'bar':  # stellar dynamical (completeness-corrected, red clump stars)
                ...
                self.sigma_ra = 0.44
                self.sigma_dec = 0.80
            elif tracer == 'carbonStars':  # wan+2020
                self.ra = 80.90
                self.dec = -68.74
                self.sigma_ra = 0.29
                self.sigma_dec = 0.12
            elif tracer == 'RGBstars':  # wan+2020
                self.ra = 81.23,
                self.dec = -69.00,
                self.sigma_ra = 0.04
                self.sigma_dec = 0.02
                self.distance = 49.9  # kpc, adopted in the paper
                ...
                self.ra = 80.27  # Rathore+2025
                self.dec = -69.65
                self.distance = 49.9  # Kacharov+2024
                self.mu_alpha = 1.88
                self.mu_delta = 0.32
                self.vlsr = 264.83
                self.inclination = 25.86
            ...
            if tracer == 'SagA*':
                ...
                self.mu_alpha = -3.156  # mas/yr
                self.mu_delta = -5.585  # reid+2020
                self.vlsr = 0  # temp
            else:
                print(f'Error: {tracer} is an invalid tracer for util/astrometry.py/Measurements/{obj}; '
                      f'line {inspect.currentframe().f_lineno}')
            ...
            if True:
                self.ra = 79.66  # Pietrzynski+1999
                self.dec = -69.41  # deg

        elif obj == 'NGC1898':
            if True:
                self.ra = 79.19  # SIMBAD
                self.dec = -69.65  # deg

        elif obj == 'NGC1835':
            if True:
                self.ra = 76.29  # SIMBAD
                self.dec = -69.41  # deg

        elif obj == 'NGC2005':
            if True:
                self.ra = 82.54,  # SIMBAD
                self.dec = -69.75  # deg

        elif obj == 'NGC2019':
            if True:
                self.ra = 82.98,  # SIMBAD
                self.dec = -70.16,  # deg
...
class LMCTangent(BaseCoordinateFrame):
    ...
    LMC = Attribute(default=Measurements('LMC', 'disk'))
...
def sun_velocity_icrs(galcen_frame=None):
    """
    Return the Sun's velocity in ICRS Cartesian coordinates (km/s).
    Extracts the pure rotation matrix between Galactocentric and ICRS
    by differencing transformed points, cancelling the origin translation.
    """
    if galcen_frame is None:
        galcen_frame = Galactocentric()

    v_sun_gc = galcen_frame.galcen_v_sun.d_xyz.to(u.km / u.s).value  # (3,)

    # Extract pure rotation by transforming pairs of points and differencing.
    # The translation cancels, leaving only the rotational part.
    basis = np.eye(3)
    origin = Galactocentric(CartesianRepresentation(0*u.kpc, 0*u.kpc, 0*u.kpc))
    origin_icrs = origin.transform_to(ICRS()).cartesian.xyz.to(u.kpc).value

    rot = np.zeros((3, 3))
    for i in range(3):
        gc_point = Galactocentric(CartesianRepresentation(
            basis[i, 0] * u.kpc,
            basis[i, 1] * u.kpc,
            basis[i, 2] * u.kpc,
        ))
        icrs_point = gc_point.transform_to(ICRS()).cartesian.xyz.to(u.kpc).value
        rot[:, i] = icrs_point - origin_icrs  # difference cancels translation

    print(f'v_sun_gc : {v_sun_gc}')
    print(f'rot @ v_sun_gc : {rot @ v_sun_gc}')
    return rot @ v_sun_gc  # (3,) km/s in ICRS Cartesian axes


# ICRS -> LMCTangent
@frame_transform_graph.transform(FunctionTransform, ICRS, LMCTangent)
def icrs_to_lmc(icrs_coord, lmc_frame):
    LMC = lmc_frame.LMC
    ...
    if icrs_coord.has_data and icrs_coord.data.differentials:  # if velocity is given
        ...
        vel = cart.differentials['s'].d_xyz.to(u.km / u.s).value

        v_sun = sun_velocity_icrs()
        vel_galcen = vel + v_sun  # now in Galactocentric-rest ICRS axes
        lmc_galcen = v_lmc(LMC) + v_sun  # LMC velocity in same frame
        vel_rel = vel_galcen - lmc_galcen  # relative to LMC
        v_dot = (R @ vel_rel) * u.km / u.s

        return LMCTangent(CartesianRepresentation(v_proj,
                                                  differentials={'s': CartesianDifferential(v_dot)}), LMC=LMC)

    return LMCTangent(CartesianRepresentation(v_proj), LMC=LMC)
...
def lmc_to_icrs(lmc_coord, icrs_frame):
    ...
    if lmc_coord.has_data and lmc_coord.data.differentials:  # if velocity is given
        ...
        # Inverse of above: unproject, add LMC velocity, subtract v_sun
        v_sun = sun_velocity_icrs()
        lmc_galcen = v_lmc(LMC) + v_sun
        vel_galcen = (R.T @ vel) + lmc_galcen  # back to Galactocentric-rest ICRS axes
        vel_icrs = vel_galcen - v_sun  # back to Sun-centred ICRS
        ...
        return ICRS(CartesianRepresentation(w * u.kpc,
                                            differentials={'s': CartesianDifferential(vel_icrs * u.km / u.s)}))
...
def lmc_to_disk(lmc_coord, disk_frame):
    ...
    if lmc_coord.has_data and lmc_coord.data.differentials:  # if velocity is given
        ...
        return LMCDisk(CartesianRepresentation(r_disk * u.kpc,
                                               differentials={'s': CartesianDifferential(v_disk)}), LMC=LMC)

    return LMCDisk(CartesianRepresentation(r_disk * u.kpc), LMC=LMC)
...
def disk_to_lmc(disk_coord, lmc_frame):
    ...
    if disk_coord.has_data and disk_coord.data.differentials:  # if velocity is given
        ...
        return LMCTangent(CartesianRepresentation(r_tan * u.kpc,
                                                  differentials={'s': CartesianDifferential(v_tan)}), LMC=LMC)

    return LMCTangent(CartesianRepresentation(r_tan * u.kpc), LMC=LMC)
