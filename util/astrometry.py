...
class Measurements:
    """
    this class holds astrometric (and photometric) measurements from the literature
    """
    def __init__(self, obj, tracer=None):
        ...
        if obj == 'LMC':
            ...
            if tracer == 'SagA*':
                ...
                self.mu_delta = -5.585  # https://iopscience.iop.org/article/10.3847/1538-4357/ab76cd
                self.vlsr = 0  # temporary
            else:
                print(f'Error: {tracer} is an invalid tracer for util/astrometry.py/Measurements/{obj}; '
                      f'line {inspect.currentframe().f_lineno}')
                exit(1)
...
class LMCDisk(BaseCoordinateFrame):
    """
    cartesian coordinate frame with the LMC disk in the x-y plane,
    the semimajor axis of the bar placed along the x-axis,
    and the axis of rotation along the +z direction
    """
...
class LMCTangent(BaseCoordinateFrame):
    """
    intermediary cartesian coordiante frame with LMC lying at origin and oriented as is in the sky
    (inclined and rotated along position angle line of nodes)
    """
    default_representation = CartesianRepresentation
    default_differential = CartesianDifferential
    LMC = Attribute(default=Measurements('LMC', 'disk'))
...
def icrs_to_lmc(icrs_coord, lmc_frame):
    ...
    if icrs_coord.has_data and icrs_coord.data.differentials:  # if velocity is given
        ...
        v_dot = (R @ (vel.value - v_lmc(LMC))) * u.km / u.s
...
def lmc_to_icrs(lmc_coord, icrs_frame):
    ...
    if lmc_coord.has_data and lmc_coord.data.differentials:  # if velocity is given
        ...
        w_dot = (R.T @ vel.value + v_lmc(LMC)) * u.km / u.s
        return ICRS(CartesianRepresentation(w * u.kpc,
                                            differentials={'s': CartesianDifferential(w_dot)}))
