...
def construct_NH0(run, subject, snap, frame, pixels, verbose=True):
    ...
    if frame == 'faux':
        ...
        smc = vrai_frame(run, smc, snap, frame, database=database, verbose=True)
        ...
        cells = vrai_frame(run, cells, snap, frame, database=database)
        if frame == 'radec':
...
def construct_PDF(run, subject, snapshot, frame, pixels=400, reDerive_lmcSnapshot=False, verbose=True):
    ...
    verbose and print(f'\t\t... +- {snapSigma} snapshots,\n'
                      f'\t\tobserver: {m[f"{database}/LMC/distance"]}, eps_BH: {1e3 * eps_BH} pc\n'
    ...
    redshifts, times = get_lookbackTimes(run, range(snapshot - snapSigma, snapshot + snapSigma))

    center_gas = calcCenter(run, subject, snapshot, 'PartType0', maxTemp=1e5, maxDist=4)
    center_gas = vrai_frame(run, {'position': center_gas},
                            snapshot, frame, database, halo=subject, verbose=False)
    center_stars = calcCenter(run, subject, snapshot, 'PartType4', maxDist=4)
    center_stars = vrai_frame(run, {'position': center_stars},
                              snapshot, frame, database, halo=subject, verbose=False)

    verbose and print('\tretrieving black holes')
    from hestia.geometry import calc_distanceDisk
    for snap in range(snapshot - snapSigma, snapshot + snapSigma):
        ...
        # bh = vrai_frame(run, bh, snap, frame, database=database)
        bh = vrai_frame(run, bh, snap, frame, database=database, halo=subject)
        e_1 = np.append(e_1, bh[frame][:, 0])  # los, in kpc
        e_2 = np.append(e_2, bh[frame][:, 1])  # dec, in deg
        e_3 = np.append(e_3, bh[frame][:, 2])  # ra, in deg
    ...
    output_base = '/z/rschisholm'
    output_path = f'/halos/{run}/{subject}/observables/bhPDF/{run}.{subject}.snap{snapshot}.bhPDF.{frame}.npz'
    np.savez(output_base + output_path, f_PDF=f_PDF, lon_edges=lon_e, lat_edges=lat_e,
             center_gas=center_gas[frame], center_stars=center_stars[frame],
...
def construct_SSD(run, subject, snap, frame, pixels, verbose=True):
    ...
    from hestia.particles import retrieve_particles
    from hestia.halos import get_halo_params
    from hestia.image import star_surface_density_projection, create_projection_histogram, create_spherical_brightness_projection
    ...
    if frame == 'faux':
        ...
        stars = vrai_frame(run, stars, snap, frame, database=database, verbose=True)
        if frame == 'radec':
    ...
    output_base = '/z/rschisholm'
    output_path = f'/halos/{run}/{subject}/observables/SSD/{run}.{subject}.snap{snap}.SSD.{frame}.npz'
...
def main(cluster):
    ...
    if args.map == 'nh0':  # H0 column densities
        ...
        output_path = construct_PDF(args.run, args.subject, int(args.snap), frame, pixels=args.pixels)
...
def plotting():
    ...
    if chisholm2025_plot:
        ...
        dispatch_plot('observables', f'{parameter}',
                      input_path=f'{basePath}/{run}.{subject}.snap{snap}.bhPDF.{frame}.npz',
                      output_path=f'{basePath}/{run}.{subject}.snap{snap}.bhPDF.{frame}.png')
