...
def create_weighted_histogram(x, y, z, bins):
    # Create a 2D histogram
    hist, x_e, y_e = np.histogram2d(x, y, bins=bins)
    # Compute the sum of metallicities in each bin
    sum_z, _, _ = np.histogram2d(x, y, bins=(x_e, y_e), weights=z)
    # Avoid division by zero
    with np.errstate(divide='ignore', invalid='ignore'):
        # Compute the average metallicity in each bin
        avg_z = np.divide(sum_z, hist, where=(hist != 0))
    return avg_z, x_e, y_e
...
def make_snap(snap_i, z, run, bins, size, axs, follow_gal=None):
    ...
    if snap_i < 100:
        ...
        snap = str(snap_i)
    key_names = ['Coordinates', 'GFM_Metallicity', 'GFM_Metals']
    ...
    particles_fini = filter_unphysical_metallicities(cosmo_transform(filter_particles(all_particles, l_b, u_b),
                                                                     'ckpc/h', 'ckpc', z))
    return create_weighted_histogram(particles_fini['Coordinates'][:, axs[0]],
                                     particles_fini['Coordinates'][:, axs[1]],
                                     np.log10(particles_fini['GFM_Metallicity'] / particles_fini['GFM_Metals'][:, 0]),
                                     bins=bins)
...
if mode == 'geras':
    if __name__ == "__main__":
        ...
        # Save data
        np.savez('/z/rschisholm/storage/images/' + simulation_run + '_gas_metallicity_' + halo + '_'
    ...
    if type_plot == 'image':
        ...
        input_path = ['/Users/dear-prudence/Desktop/smorgasbord/images/metallicity/10x80/'
                      '09_18_gas_metallicity_lmc_10x80ckpc_bin0.2ckpc_0_1.npz',
                      '/Users/dear-prudence/Desktop/smorgasbord/images/metallicity/10x80/'
                      '09_18_gas_metallicity_lmc_10x80ckpc_bin0.2ckpc_2_1.npz']
        output_path = '/Users/ursa/Desktop/smorgasbord/images/metallicity/10x80/'

        ursa.plot_frames('metallicity', input_path, output_path, scale=[10, 80])
