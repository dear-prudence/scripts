...
def make_plot(data, snaps):
    ...
    if len(data['time_edges']) == 1:
        ...
        for ax, snap in zip(axes, snaps):
            ...
            ax.set_title('$z = $' + '{:.{}f}'.format(dates[frame, 0], 3)
                         ...
                         + ' Gyr',
                         x=0.75, y=0.005, ha='center', va='bottom', weight='bold', fontsize='small')
            frame += 1
    ...
    plt.savefig('/Users/dear-prudence/Desktop/smorgasbord/radialProfiles/09_18_radialProfiles_'
                + ('snapsMultiple' if isinstance(snaps, list) else 'snap' + str(snap)) + '_lmc.png', dpi=240)
    plt.show()
