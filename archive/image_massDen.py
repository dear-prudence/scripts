...
from hestia import filter_particles
from hestia import cosmo_transform
from hestia import time_edges
from hestia import center_lmc
...
def make_snap(snap_i, z, run, part_type, bins, size, axs, follow_gal=None, iteration=True):
    ...
    key_names = ['Coordinates', 'Masses']  # use 'Density' for gas particles,
    # need to change create_weighted_histogram call line
    if iteration is False:
        ...
        return create_weighted_histogram(lmc_particles['Coordinates'][:, axs[0]],
                                         ...
                                         densities=lmc_particles['Masses'], bins=bins, bounds=bounds)
    else:
        ...
        filtered_particles = cosmo_transform(filter_particles(all_particles, l_b, u_b),
                                             'ckpc/h', 'ckpc', z)
        return create_weighted_histogram(filtered_particles['Coordinates'][:, axs[0]],
                                         filtered_particles['Coordinates'][:, axs[1]],
                                         densities=filtered_particles['Masses'], bins=bins)
...
if mode == 'geras':
    if __name__ == "__main__":
        ...
        if isinstance(snaps, int) or len(snaps) == 1:
            ...
            np.savez('/z/rschisholm/storage/images/' + simulation_run + '_'
                     + ('gas' if particle == 'PartType0' else 'stars') + '_massDen_' + halo + '_'
                     + str(dims[0]) + 'x' + str(dims[1]) + 'ckpc_'
                     + 'bin' + str(round(spatial_size[axes[0]] / float(n_bins[0]), 2)) + 'ckpc_'
                     + str(axes[0]) + '_' + str(axes[1]) + '.npz',
    ...
    # ------------------------------------
    type_plot = 'combo_frames'
    # ------------------------------------

    if type_plot == 'image':
        input_path = ('/Users/dear-prudence/Desktop/smorgasbord/images/massDen_lmc_200x1600ckpc/'
                      '09_18_gas_massDen_LMC_100x800x800ckpc_bin1.0ckpc_2_1.npz')
        output_path = ('/Users/dear-prudence/Desktop/smorgasbord/images/massDen_LMC_100x800ckpc/'
        ...
        ursa.plot_image('massDen', input_path, output_path, snaps=snaps, scale=[50, 400])

    elif type_plot == 'frames':
        input_paths = ['/Users/dear-prudence/Desktop/smorgasbord/images/stars/'
                       '09_18_stars_massDen_lmc_50x400ckpc_bin1.0ckpc_2_1.npz',
                       '/Users/dear-prudence/Desktop/smorgasbord/images/stars/'
                       '09_18_stars_massDen_lmc_50x400ckpc_bin1.0ckpc_2_1.npz']
        output_path = '/Users/ursa/Desktop/smorgasbord/images/stars/snaps/'

        ursa.plot_frames('massDen', input_paths, output_path, scale=[50, 400])

    elif type_plot == 'combo_frames':
        input_paths = ['/Users/dear-prudence/Desktop/smorgasbord/images/temperature/100x800ckpc/'
                       '09_18_gas_temperature_LMC_100x800x800ckpc_bin2.0ckpc_2_1.npz',
                       '/Users/dear-prudence/Desktop/smorgasbord/images/velMag/'
                       '09_18_gas_velocityMag_lmc_100x800ckpc_bin2.0ckpc_2_1.npz']
        output_path = '/Users/ursa/Desktop/smorgasbord/images/combination_temperature_velocity_100x800/'
