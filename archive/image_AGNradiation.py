...
def create_weighted_histogram(x, y, radiation, bins):
    threshold = 1
    # Create a 2D histogram
    hist, x_e, y_e = np.histogram2d(x, y, bins=bins)

    # Compute the sum of temperatures in each bin
    sum_rads, _, _ = np.histogram2d(x, y, bins=(x_e, y_e), weights=radiation)
    # Avoid division by zero
    with np.errstate(divide='ignore', invalid='ignore'):
        # Compute the average temperature in each bin
        avg_rads = np.divide(sum_rads, hist, where=(hist != 0))
    avg_rads[hist < threshold] = 0  # sets bins with only one particle to zero average AGN (in an attempt to clean)
    return avg_rads, x_e, y_e
...
def make_snap(snap_i, z, run, bins, size, axs, follow_gal=None):
    ...
    if snap_i < 100:
        ...
        snap = str(snap_i)
    key_names = ['Coordinates', 'GFM_AGNRadiation']
    ...
    particles_fini = cosmo_transform(filter_particles(all_particles, l_b, u_b),
                                     'ckpc/h', 'ckpc', z)

    return create_weighted_histogram(particles_fini['Coordinates'][:, axs[0]],
                                     particles_fini['Coordinates'][:, axs[1]],
                                     particles_fini['GFM_AGNRadiation'], bins=bins)
...
if mode == 'geras':
    if __name__ == "__main__":
        ...
        axes = [0, 1]
        n_bins = [400, 400]
        snaps = [124, 127]  # first and last snapshot of the series to be compiled (i.e. bounds in time)
        snap_stepSize = 1
        halo = 'lmc'
        # -------------------
        ...
        # Save data
        np.savez('/z/rschisholm/storage/images/' + simulation_run + '_gas_AGNradiation_' + halo + '_'
    ...
    if type_plot == 'image':
        ...
        input_path = ['/Users/dear-prudence/Desktop/smorgasbord/images/AGNradiation/'
                      '09_18_gas_AGNradiation_lmc_50x400ckpc_bin1.0ckpc_0_1.npz',
                      '/Users/dear-prudence/Desktop/smorgasbord/images/AGNradiation/'
                      '09_18_gas_AGNradiation_lmc_50x400ckpc_bin1.0ckpc_2_1.npz']
        output_path = '/Users/ursa/Desktop/smorgasbord/images/AGNradiation/'

        ursa.plot_frames('AGNradiation', input_path, output_path, scale=[50, 400])
