...
def create_weighted_histogram(x, y, velocities, bins):
    ...
    with np.errstate(divide='ignore', invalid='ignore'):
        ...
        avg_vels = np.divide(sum_vels, hist, where=(hist != 0))
    return avg_vels, x_e, y_e
...
def make_snap(snap_i, z, run, bins, size, axs, follow_gal=None):
    ...
    _, _, lmc_vel, _ = get_halo_params('lmc', snap_i)
    vel_mags = np.array([])
    for j in range(len(particles_fini['Velocities'])):
        vel_mags = np.append(vel_mags, np.linalg.norm(particles_fini['Velocities'][j] - lmc_vel))
...
if mode == 'geras':
    if __name__ == "__main__":
        ...
        axes = [2, 1]
        n_bins = [400, 400]
        snaps = [125, 127]  # first and last snapshot of the series to be compiled (i.e. bounds in time)
        snap_stepSize = 1
        halo = 'lmc'
        # -------------------
        ...
        # Save data
        np.savez('/z/rschisholm/storage/images/' + simulation_run + '_gas_velocityMag_' + halo + '_'
    ...
    if type_plot == 'image':
        ...
        input_path = ['/Users/dear-prudence/Desktop/smorgasbord/images/velMag/'
                      '09_18_gas_velocityMag_lmc_50x400ckpc_bin1.0ckpc_2_1.npz',
                      '/Users/dear-prudence/Desktop/smorgasbord/images/velMag/'
                      '09_18_gas_velocityMag_lmc_50x400ckpc_bin1.0ckpc_2_1.npz']
        output_path = '/Users/ursa/Desktop/smorgasbord/images/velMag/'

        ursa.plot_frames('velocity', input_path, output_path, scale=[50, 400])
