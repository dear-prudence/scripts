...
def make_snap(snap, num_bins):
    ...
    size = np.array([600, 600, 150]) * 10 ** -3  # array in kpc
    center = center_mw(snap, filename='/store/clues/galaxy/RE_SIMS/8192/GAL_FOR/09_18/AHF_output_2x2.5Mpc'
    ...
    x_data = np.array(processed_data['Coordinates'])[:, 0]
    y_data = np.array(processed_data['Coordinates'])[:, 1]

    ra = [[min_bound[0], max_bound[0]], [min_bound[1], max_bound[1]]]
    hist, x_, y_ = create_weighted_histogram(x_data, y_data, processed_data['Temperature'], ranges=ra, num_bins=num_bins)
    processed_data = reduce_graininess(hist, threshold=3 * 10 ** 7)
...
if __name__ == "__main__":
    ...
    # Save data
    print('Saving Data...')
    np.savez('/z/rschisholm/storage/09_18_snap121_MW_temperature_600x600kpc.npz',
